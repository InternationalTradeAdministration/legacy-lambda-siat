...
def handler(workbook, workbook_name, workbook_uri):
  data = []
  
  
  entry = collections.OrderedDict()
  header_row_index = set_header_row_index(workbook)
  double_row_header = set_double_row_header(workbook)
  entry['type'] = set_survey_type(workbook)
  
  
  match_group = re.search(r'[0-9]{4}', workbook_name)
  if not match_group:
    return
  ...
  write_csv_file(data, workbook_name, temp_file)

  new_blob_name = 'translated/'+workbook_name[9:].replace('xlsx', 'csv')
  block_blob_service = bbs(account_name = acct_name, account_key = acct_key)
  with open(temp_file.name, 'r+') as upload_data:
    block_blob_service.create_blob_from_text(container_name = "siat", blob_name = new_blob_name, text = str(upload_data.read()))
